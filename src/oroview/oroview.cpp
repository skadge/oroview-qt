...
void OroView::update(float t, float dt) {
    ...
    //TODO SDL_Delay(20); //N'allons pas trop vite au début...
    ...
    //TODO logic_time = SDL_GetTicks();

    logic(runtime, dt);

    //TODO logic_time = SDL_GetTicks() - logic_time;

    //TODO draw_time = SDL_GetTicks();
...
void OroView::mouseTrace(Frustum& frustum, float dt) {
    ...
    //TODO gluPerspective(90.0f, (GLfloat)display.width/(GLfloat)display.height, 0.1f, camera.getZFar());
    camera.look();
...
//    }
    ...
    //TODO trace_time = SDL_GetTicks();

    mouseTrace(frustum,dt);

    //TODO trace_time = SDL_GetTicks() - trace_time;
...
//	userTree->outline();
    ...
    //TODO fontmedium.draw(display.width/2 - date_x_offset, 20, displaydate);
	...
	track_users ? usersBounds.draw() : nodesBounds.draw();
        ...
        //TODO font.print(0,20, "FPS: %.2f", fps);
...
//        font.print(0,60,"Users: %d", users.size());
	...
	font.print(0,200,"Mouse Trace: %u ms", trace_time);
        //TODO font.print(0,220,"Draw Time: %u ms", SDL_GetTicks() - draw_time);
...
void OroView::drawBackground(float dt) {
    //TODO display.setClearColour(background_colour);
    //TODO display.clear();
}
...
void OroView::loadingScreen() {
    ...
    //TODO font.print(display.width/2 - width/2, display.height/2 - 10, "%s", loading_message.c_str());
}
...
void OroView::selectBackground() {
    ...
    //TODO gluPerspective(90.0f, (GLfloat)display.width/(GLfloat)display.height, 0.1f, -camera.getPos().z);
    camera.look();

    //TODO vec2f screen_centre(display.width*0.5,display.height*0.5);
    vec2f screen_centre(100,100);

    //TODO backgroundPos = display.unproject(screen_centre).truncate();
